...
import { RegisterLeaveFormValues } from '../RegisterLeave/useRegisterLeave';

export type DailyDeclaration = { morning: string; afternoon: string };

export const registerOvertimeSchema = yup.object({
...
export type RegisterOvertimeFormValues = Omit<
  yup.InferType<typeof registerOvertimeSchema>,
  'dailyDeclarations'
> & {
...
export const useRegisterOvertime = () => {
  const { openCalendarPicker } = useCalendarPicker();
  const { openTimePicker } = useTimePicker();

  const initialValues: RegisterOvertimeFormValues = {
    fromDate: '',
    toDate: '',
    reason: '',
