  ...
  if (navigationRef.isReady()) {
    (
      navigationRef.navigate as (
        n: RouteName,
        p?: RootStackParamList[RouteName],
      ) => void
    )(name, params);
  }
}
...
export function reset(
  state:
    | {
        index: number;
        routes: Array<{ name: keyof RootStackParamList; params?: unknown }>;
      }
    | ReturnType<typeof CommonActions.reset>['payload'],
) {
  if (navigationRef.isReady()) {
    navigationRef.dispatch(
      CommonActions.reset(state as Parameters<typeof CommonActions.reset>[0]),
    );
  }
}
